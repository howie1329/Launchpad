...
import { deleteAccountMutation, getAccountResetCleanupTargetsQuery } from '$lib/account-management';
	...
	try {
		...
		if (memoryDelete.status === 'failed') {
			return json(
				{
					ok: false,
					error: 'Could not clear remote memory. Your account was not deleted. Please try again.'
				},
				{ status: 502 }
			);
		}

		await convex.mutation(deleteAccountMutation, {});
		return json({ ok: true });
	} catch (error) {
		console.error(error);
		return json({ ok: false, error: 'Could not delete account' }, { status: 500 });
	}
};
