...
import { requireAuthUserId, getOptionalAuthUserId } from './authHelpers'
...
function targetsEqual(a: WorkspaceTabTarget, b: WorkspaceTabTarget): boolean {
...
async function assertTargetAccess(ctx: MutationCtx, ownerId: string, target: WorkspaceTabTarget) {
	if (target.kind === 'home' || target.kind === 'settings') return
	if (target.kind === 'project') {
		const p = await ctx.db.get(target.projectId)
		if (!p || (p as { ownerId: string }).ownerId !== ownerId) {
	...
	if (target.kind === 'artifact') {
		...
		if (!a || (a as { ownerId: string }).ownerId !== ownerId) {
...
export const addOrActivate = mutation({
	...
	handler: async (ctx, { target }): Promise<{ tabId: string }> => {
		...
		const ownerKey = ownerId as unknown as string
		await assertTargetAccess(ctx, ownerKey, target as WorkspaceTabTarget)
		...
		const idx = existing.tabs.findIndex((t) =>
			targetsEqual(t.target as WorkspaceTabTarget, target as WorkspaceTabTarget)
		)
		if (idx >= 0) {
		...
		const tabs = [
			...existing.tabs,
			{ id: tabId, target: target as WorkspaceTabTarget }
		]
...
export const removeTab = mutation({
	...
	handler: async (ctx, { tabId }) => {
		...
		const doc = await ctx.db
			...
			.withIndex('by_ownerId', (q) => q.eq('ownerId', ownerKey))
			.unique()
		if (!doc) return { removed: false as const }

		const i = doc.tabs.findIndex((t) => t.id === tabId)
		if (i < 0) return { removed: false as const }
		...
		if (!wasActive) {
			return { removed: true as const, navigate: false as const }
		}
