...
{#snippet triggerLeftIcon()}
	<span class="relative inline-flex size-4 items-center justify-center text-muted-foreground">
		...
		{:else}
			<CheckIcon class="size-4 text-emerald-600 dark:text-emerald-500" aria-hidden="true" />
		{/if}
	</span>
{/snippet}

<div role="region" aria-label="Assistant tool activity" class="max-w-full min-w-0">
	<Steps bind:open class="rounded-lg border border-border/60 bg-muted/20">
		<StepsTrigger class="px-3 py-2.5" leftIcon={triggerLeftIcon}>
			<span class="font-medium text-foreground">{triggerLabel}</span>
		</StepsTrigger>
		<StepsContent class="px-3 pb-3">
				...
				<StepsItem class="space-y-1.5">
					<div class="flex items-start gap-2">
						...
						<div class="min-w-0 flex-1 space-y-1">
							<p class="text-sm leading-snug font-medium text-foreground">{tool.title}</p>
							...
							{/if}
							<details class="group/details pt-0.5">
								<summary
									class="cursor-pointer text-[11px] text-muted-foreground underline-offset-2 hover:underline"
								>
									Technical details
								</summary>
								<pre
									class="mt-1.5 max-h-40 overflow-x-auto overflow-y-auto rounded-md border border-border/50 bg-background/80 p-2 font-mono text-[10px] leading-relaxed text-muted-foreground">{tool.detailJson}</pre>
							</details>
						</div>
					</div>
				</StepsItem>
