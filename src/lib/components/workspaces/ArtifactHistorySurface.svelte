...
<div class={compact ? 'space-y-4' : 'min-h-[calc(100vh-13rem)]'}>
	{#if versions.length === 0}
		<div
			class="flex min-h-64 items-center justify-center rounded-lg border border-border/70 bg-background p-6 text-center"
		>
			<div class="max-w-sm">
				<p class="text-sm font-medium text-foreground">No saved versions yet</p>
				<p class="mt-1 text-xs leading-5 text-muted-foreground">
					Saved versions will appear after this artifact is edited.
				</p>
			</div>
		</div>
	{:else}
		<div
			class={compact
				? 'space-y-4'
				: 'grid min-h-[calc(100vh-13rem)] overflow-hidden rounded-lg border border-border/70 bg-background lg:grid-cols-[18rem_minmax(0,1fr)]'}
		>
			<section
				class={compact
					? 'space-y-2'
			...
			>
				<div class={compact ? 'px-1' : ''}>
					...
					<p class="mt-1 text-xs leading-5 text-muted-foreground">
						Every saved edit becomes a version you can compare or restore.
					</p>
				</div>
				...
				{#if selectedVersion}
					...
					>
						<div class="space-y-1">
							...
							<p class="text-xs text-muted-foreground">
								{#if compareBaseVersion}
									Comparing against version {compareBaseVersion.versionNumber}.
								{:else}
									No earlier version selected for comparison.
								{/if}
							</p>
						</div>
						...
						<div class="flex flex-wrap items-center gap-2">
							<div
								class="inline-flex items-center rounded-md border border-border/70 bg-background p-0.5"
							>
								<Button
									type="button"
									size="sm"
							...
							>
								{restoringVersionId === selectedVersion._id ? 'Restoring…' : 'Restore this version'}
							</Button>
						</div>
					</div>
