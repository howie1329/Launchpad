				...
				<section class="rounded-lg border border-border/70 bg-card/35 p-3">
							...
							<div class="flex gap-1.5">
								...
								>
									{#if triggerTypes.length === 0}
										<option value="">Trigger slug</option>
									{:else}
										{#each triggerTypes as trigger (trigger.slug)}
											<option value={trigger.slug}>{trigger.name}</option>
										{/each}
									{/if}
								</select>
								<button
					...
					<div class="mt-2 flex flex-wrap items-center justify-between gap-2">
						<div class="min-h-4 text-[11px]">
							{#if triggerTypesLoading}
								<span class="text-muted-foreground">Loading Composio triggers…</span>
							{:else if triggerTypesError}
								<span class="text-destructive">{triggerTypesError}</span>
							{:else if actionError}
								<span class="text-destructive">{actionError}</span>
							{:else if actionNotice}
								<span class="text-muted-foreground">{actionNotice}</span>
							{/if}
						</div>
						<button
							type="button"
						...
						>
							<HugeiconsIcon icon={PlayIcon} strokeWidth={2} class="size-3.5" />
					...
					<div class="mt-3 border-t border-border/70 pt-2">
						{#if launchpadActions.data === undefined}
							<p class="px-1 py-3 text-xs text-muted-foreground">Loading actions…</p>
						{:else if projectActions.length === 0}
							<p class="px-1 py-3 text-xs leading-5 text-muted-foreground">
								No Launchpad Actions yet.
							</p>
						{:else}
							<div class="space-y-1">
